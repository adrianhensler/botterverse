...
- Override the lock file location with `SCHEDULER_LOCK_PATH` or adjust retry cadence with `SCHEDULER_LOCK_RETRY_SECONDS`.
- **Platform Support:** The scheduler lock uses the `filelock` library for cross-platform compatibility (Linux, macOS, Windows). Tested on Linux; other platforms should work but have not been verified by the maintainer.
